class Resume < ActiveRecord::Base
  ...
  def parse(cv_document)
    ...
    name = File.join(file_support.root, 'file_assets',cv_document)
  	wsdl='http://java.rchilli.com/RChilliParser/services/RChilliParser?wsdl'
		resume_file_data = File.read(name)
    ...
    self.xml_response = xml.body[:parse_resume_binary_response][:return]
    self.add_file(resume_file_data, name)
    self.save
  end

  def populate_model_data
  	doc = Nokogiri.XML(self.xml_response)
  	doc_root = doc.root
  	...
  	phone = PhoneNumber.create(phone_number: doc_root.at_xpath("Phone").text)
  	...
  	skills.each do |skill|
  		skill_type = SkillType.create(description: skill)
