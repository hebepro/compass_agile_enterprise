Ext.define('Compass.ErpApp.Desktop.Applications.JobTracker.Model', {
    ...
    fields: [
        ...
        {name: 'scheduled', type: 'boolean', mapping: 'additional_values.scheduled'},
        {name: 'last_run_at', type: 'date', dateFormat: 'm-d-Y H:i:s', mapping: 'additional_values.last_run_at'},
        {name: 'next_run_at', type: 'date', dateFormat: 'm-d-Y H:i:s', mapping: 'additional_values.next_run_at'}
    ]
});
    ...
    columns: [
        ...
        },
        {
            ...
            renderer: function (value) {
                return (Ext.isEmpty(value) ? 'N/A' : Ext.Date.format(value, 'm-d-Y h:i:s'));
            },
            width: 150
        },
        {
            ...
            items: [
                {
                    ...
                    getTip:function( value, metadata, rec ) {
                        var now = new Date();
                        var value = rec.get('next_run_at');
                        if (!Ext.isEmpty(value) && (now > value)) {
                            return 'Warning';
