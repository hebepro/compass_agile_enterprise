...
inner join applications on applications.id = app_containers_applications.application_id')
      ...
      ActiveRecord::Base.connection.execute("delete from applications where type = 'MobileApplication'")

      # remove type since there is only DesktopApplications and Applications now
